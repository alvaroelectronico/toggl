...
df_summary = df_toggl[['lunes_semana', 'client', 'project', 'h_registradas']].groupby(by=['lunes_semana', 'client', 'project'], as_index=False).sum()

asignacion_path = "../data/asignacion.xlsx"
df_asig = xl.get_asignacion(asignacion_path)

df_toggl_asig = pd.merge(df_asig, df_summary, how='outer', on=['lunes_semana', 'client', 'project'])


df = io.read_cache_toggl(path, start_date, end_date)
