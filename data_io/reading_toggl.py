...
def get_toggl(toggl, start_date="2021-09-03", end_date="2021-09-03"):
    ...
    if len(time_entries) == 0:
        df_toggl = pd.DataFrame(columns=['date', 'client', 'project', 'h_registradas', 'description', 'start', 'lunes_semana'])
        df_toggl
        return df_toggl

    entries = list()
    for i in time_entry_ids:
        entries.append(toggl.request("https://api.track.toggl.com/api/v8/time_entries/{}".format(i))['data'])
    ...
    df_cid_pid_dict = {'pid': dct_cid_pid.keys(),
    ...
    df_toggl = pd.merge(df_toggl, df_cid_pid[['pid', 'project', 'cid']], on=['pid'], how='left')
    ...
    df_toggl["lunes_semana"] = df_toggl["lunes_semana"].astype(
        "datetime64[ns]"
    )


    df_toggl = df_toggl[['date', 'client', 'project', 'h_registradas', 'description', 'start', 'lunes_semana']]
